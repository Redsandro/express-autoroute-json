...
module.exports = (options) => async(req, res, next) => {
	const pageLimit		= Math.min(+get(req.query, 'page.limit'), Math.max(options.args.limitMax || 0, 100))
	...
	const find			= options.model.find(match).select(req.jsMini.select).sort(req.jsMini.sort).lean(false).limit(limit)
	...
	try {
		...
		res.json(serializer(req, results, options))
		return next()
	}
	catch (err) {
		return next(err)
