...
module.exports = (options) => async(req, res, next) => {
	...
	try {
		if (options.find.query) {
			 ...
			 */
			// TODO: select only _id and redo pipeline
			const size = +sample || 1
			results = await Model.aggregate().match(match).sample(size).project({_id: 1}).exec()
			results = results.map(m => m._id)
			results = Model.find({_id: {$in: results}}).lean(false)
			results = relInclude.reduce((findOne, rel) => findOne.populate(rel), results)
